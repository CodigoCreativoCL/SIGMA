...
(function () {
    ...
    function findEdit(row) {
        ...
        return one('.sgx-original-edit, .icono_Editar, a[onclick*="abrir"]', row);
    }
    ...
    function addDetailTrigger(row) {
        ...
        trigger.setAttribute('data-sgx-action', 'detail');
        trigger.setAttribute('aria-label', 'Ver detalle de ' + rowTitle(row, rowFields(row)));
        trigger.setAttribute('title', 'Ver detalle');
    ...
    function alignDrawer() {
        ...
        var offset = Math.max(0, Math.round(listBox.top - hostBox.top));

        drawer.style.setProperty('--sgx-drawer-offset', offset + 'px');
        drawer.style.setProperty('--sgx-drawer-height', Math.round(listBox.height) + 'px');
    }
    ...
    function clickHandler(event) {
        ...
        if (action && host && host.contains(action)) {
            ...
            else if (name === 'detail') openRow(closest(action, '.sgx-detail-row'));
