(function (window, document) {
    ...
    function recordNameFromDocument(doc) {
        if (!doc) return '';
        var preferred = doc.querySelector(
            '[data-sigma-record-name], input[id*="txtNombre"], input[id*="txtRazonSocial"], ' +
            'input[id*="txtDescripcion"], input[id*="txtCodigo"], input[id*="txtFolio"]'
        );
    ...
    function minimize(forceMinimize) {
        ...
        if (window.gsap) {
            ...
            gsap.fromTo(surface,
                { opacity: .35, y: willMinimize ? 24 : 12, scale: .94 },
                { opacity: 1, y: 0, scale: 1, duration: .38, ease: 'power3.out', clearProps: 'opacity,transform' });
        }
