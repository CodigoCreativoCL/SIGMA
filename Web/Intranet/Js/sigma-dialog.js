(function (window, document) {
    ...
    /* ========================================================================
       EL MODAL DEJA DE CRECER SOLO

       EL SINTOMA

         Al abrir una ventana, el marco crecia despacio "desde la nada" hasta
         su tamaño, como si se estuviera inflando. No era una animacion: era
         el alto persiguiendo al contenido.

       LA CAUSA

         El MutationObserver de aca abajo escuchaba `attributes: true` con
         `subtree: true`, o sea CUALQUIER cambio de atributo en cualquier
         elemento de la pagina de adentro. GSAP anima escribiendo `style` en
         linea, sesenta veces por segundo. Cada una de esas escrituras
         disparaba una medicion, y cada medicion movia el alto del marco un
         poquito. Con las animaciones de entrada del contenido —el calendario,
         los pasos, los chips— eso son cientos de mediciones seguidas, cada
         una un par de pixeles mas alta que la anterior. Se veia crecer.

       LA CORRECCION

         Dos cosas. Los atributos dejan de observarse: un cambio de estilo no
         es un cambio de contenido, y el tamaño real ya lo vigila el
         ResizeObserver, que es para eso. Y la medicion se agrupa: una rafaga
         de cambios termina en UNA medicion cuando la rafaga para, no en una
         por cambio.
       ======================================================================== */
    var medirPendiente = null;

    function measureSoon() {
        if (medirPendiente) window.clearTimeout(medirPendiente);

        medirPendiente = window.setTimeout(function () {
            medirPendiente = null;
            measure();
        }, 90);
    }
    ...
    function clearObservers() {
        /* Si quedaba una medicion agendada, sobrevive al cierre y se ejecuta
           sobre un marco que ya no muestra nada. */
        if (medirPendiente) {
            window.clearTimeout(medirPendiente);
            medirPendiente = null;
        }
    ...
    function observeCurrentFrame() {
        ...
        try {
            ...
            if (window.MutationObserver && frame.contentDocument.body) {
                mutationObserver = new MutationObserver(measureSoon);
                mutationObserver.observe(frame.contentDocument.body, {
                    /* Solo cuando aparece o desaparece contenido. `attributes`
                       incluia cada `style` que escribe GSAP mientras anima, y
                       eso hacia crecer la ventana de a pixeles durante toda la
                       animacion de entrada. El tamaño real lo vigila el
                       ResizeObserver de arriba. */
                    childList: true,
                    subtree: true,
                    attributes: false,
                    characterData: false
