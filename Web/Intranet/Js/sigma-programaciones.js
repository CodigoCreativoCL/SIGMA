...
(function () {
    ...
    function apply() {
        ...
        /* ------------------------------------------------------------------
           ...
           Aca decia: si no hay ninguna fila abierta, abri la primera. La
           pantalla arrancaba con el detalle de una programacion que nadie
           pidio, y el resto de los paneles del sistema arrancan cerrados. Ese
           detalle ocupaba 360px de ancho desde el primer segundo, empujando
           la lista, y daba a entender que esa fila estaba seleccionada.

           Se abre cuando se elige una fila. Lo unico que se conserva es que
           si la fila abierta sigue estando despues de filtrar u ordenar, se
           queda abierta: cerrar el panel al escribir en el buscador seria
    ...
    /* ========================================================================
       ...
       Es la misma que usan los paneles de Activos, Proveedores, Bodegas y
       Permisos —`sigma-listas.js`—, con los mismos tiempos y la misma curva.
       Aca el panel solo aparecia y desaparecia con una transicion CSS: dos
       paneles que hacen lo mismo moviendose distinto se sienten como dos
       productos.

       El panel entra desde la derecha y su contenido detras, escalonado. Al
       saltar de una fila a otra el panel NO vuelve a entrar —ya esta ahi—:
       solo se renueva el contenido. Volver a animar la caja entera la haria
       saltar en su sitio en cada clic.
       ======================================================================== */
    var drawerTimeline = null;

    function drawerItems(drawer) {
        return all('.sgp-drawer-content > *, .sgp-drawer-actions > *', drawer);
    }

    function animateDrawer(drawer, wasOpen) {
        if (!window.gsap) return;

        var items = drawerItems(drawer);
        ...
        if (!wasOpen) {
            drawerTimeline.fromTo(drawer,
                ...
                { autoAlpha: 1, x: 0, scale: 1, duration: .56, ease: 'power4.out',
                  clearProps: 'opacity,visibility,transform' },
                0);
        }

        drawerTimeline.fromTo(items,
            { autoAlpha: 0, x: wasOpen ? 12 : 0, y: wasOpen ? 2 : 11 },
            { autoAlpha: 1, x: 0, y: 0, duration: .34, stagger: .035, ease: 'power3.out',
              clearProps: 'opacity,visibility,transform' },
            wasOpen ? 0 : .14);
    }
    ...
    function openRow(row) {
        ...
        var wasOpen = root.classList.contains('has-drawer');
        ...
        /* Se anima en el siguiente cuadro: el contenido acaba de entrar al DOM
           y todavia no tiene medidas. */
        if (window.gsap) window.requestAnimationFrame(function () { animateDrawer(drawer, wasOpen); });
    }
    ...
    function closeDrawer(immediate) {
        ...
        function terminar() {
            ...
            if (window.gsap) gsap.set(drawer, { clearProps: 'opacity,visibility,transform' });
        }

        if (!immediate && window.gsap && root.classList.contains('has-drawer')) {
            if (drawerTimeline) drawerTimeline.kill();
            gsap.killTweensOf(drawer);
            drawerTimeline = gsap.timeline({ onComplete: terminar });
            drawerTimeline
                .to(drawerItems(drawer), {
                    autoAlpha: 0, y: -5, duration: .15, stagger: .018, ease: 'power2.in'
                }, 0)
                .to(drawer, {
                    autoAlpha: 0,
