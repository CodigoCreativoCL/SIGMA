<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Paises.ascx.cs" Inherits="View_Sistema_Usuarios_Controls_Paises" %>

<script type="text/javascript">
    function abrirPaises(query) {
        return SigmaModal.open({
            url: '<%=ResolveUrl("~/View/Root/Mantenedores/Usuarios/UsuarioPaises.aspx") %>?query=' + query,
            title: 'Usuario paises',
            width: 1000,
            initialHeight: 380
        });
    }
...
<asp:UpdatePanel runat="server" ID="udPanel" UpdateMode="Conditional">
    <ContentTemplate>
        <rad:RadGrid2 ID="Grid" runat="server">
            <MasterTableView CommandItemDisplay="Top" DataKeyNames="upa_id">
                <CommandItemTemplate>
                    <div style="margin-bottom: 10px;">
                        ...
                        <asp:LinkButton ID="lnkEliminar" runat="server" Text="Desvincular" CssClass="icono_eliminar" OnClick="lnkEliminar_Click"
                            OnClientClick="return ConfirSweetAlert(this, '', '¿Está seguro que desea desvincular los paises seleccionados?');" />
                    </div>
                </CommandItemTemplate>
            </MasterTableView>
        </rad:RadGrid2>
