...
-- DESCRIPTION:     ES_FAVORITO EN LOS LISTADOS DE TERRENO.
-- =============================================
-- Va DESPUES de BD/180, que crea `Usuario_Favorito`.
--
-- La estrella se resuelve en el MISMO SELECT del listado y no en una consulta
-- aparte: pedir los favoritos por separado haria que las estrellas aparecieran
-- despues de las tarjetas, encendiendose de a una sobre una lista ya dibujada.
--
            ...
            ,CAST(CASE WHEN EXISTS (
                SELECT 1 FROM [dbo].[Usuario_Favorito] ufv
                 WHERE ufv.[ufv_usuario] = @USUARIO
                   AND ufv.[ufv_orden_trabajo] = otr.[otr_id]) THEN 1 ELSE 0 END AS BIT)
                                                   AS [ES_FAVORITO]
