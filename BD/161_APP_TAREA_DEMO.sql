...
--  El bloque se puede volver a correr: se borra lo que dejo la corrida
--  anterior y se vuelve a empezar. Se borra **solo lo de prueba** -las
--  ejecuciones, los comentarios y los dictados de estas cinco tareas-, no la
--  tabla entera.
DELETE tco FROM [dbo].[Tarea_Comentario] tco
  JOIN [dbo].[Tarea_Ocurrencia] toc ON toc.[toc_id] = tco.[tco_tarea_ocurrencia]
  JOIN [dbo].[Tarea] t ON t.[tar_id] = toc.[toc_tarea] WHERE t.[tar_codigo] LIKE 'TAR-00%'
DELETE FROM [dbo].[Dictado_Voz] WHERE [dvo_uuid] LIKE '9E1C3A70-0002-%'
DELETE av FROM [dbo].[Archivo_Vinculo] av
  JOIN [dbo].[Tarea_Ejecucion] tej ON tej.[tej_id] = av.[avi_tarea_ejecucion]
  JOIN [dbo].[Tarea_Ocurrencia] toc ON toc.[toc_id] = tej.[tej_tarea_ocurrencia]
  JOIN [dbo].[Tarea] t ON t.[tar_id] = toc.[toc_tarea] WHERE t.[tar_codigo] LIKE 'TAR-00%'
DELETE FROM [dbo].[Archivo] WHERE [arc_uuid] LIKE '9E1C3A70-0004-%'
DELETE tej FROM [dbo].[Tarea_Ejecucion] tej
  JOIN [dbo].[Tarea_Ocurrencia] toc ON toc.[toc_id] = tej.[tej_tarea_ocurrencia]
  JOIN [dbo].[Tarea] t ON t.[tar_id] = toc.[toc_tarea] WHERE t.[tar_codigo] LIKE 'TAR-00%'
DELETE toa FROM [dbo].[Tarea_Ocurrencia_Asignacion] toa
  JOIN [dbo].[Tarea_Ocurrencia] toc ON toc.[toc_id] = toa.[toa_tarea_ocurrencia]
  JOIN [dbo].[Tarea] t ON t.[tar_id] = toc.[toc_tarea] WHERE t.[tar_codigo] LIKE 'TAR-00%'
UPDATE toc SET [toc_tarea_ocurrencia_estado] = 1
...
PRINT ''
PRINT '== COMENTARIOS =='
--  Dictado: el texto crudo dice «marca inferior no relleno aviso» -sin
--  puntuacion, como sale del reconocedor- y el comentario es lo que Cristian
--  corrigio antes de mandarlo. Guardar los dos es lo unico que despues
--  permite saber si dictar en la sala de maquinas sirve o no.
EXEC [dbo].[API_INS_TAREA_COMENTARIO]
    ...
    ,@DICTADO_UUID      = '9E1C3A70-0002-4A00-9000-000000000001'
    ,@TEXTO_DICTADO     = N'el nivel esta justo en la marca inferior no relleno aviso'
    ,@DICTADO_CONFIANZA = 0.8710
    ,@DICTADO_SEGUNDOS  = 6
...
PRINT '== COMENTARIO REPETIDO (no debe duplicar ni el dictado) =='
EXEC [dbo].[API_INS_TAREA_COMENTARIO]
     @OCURRENCIA = @OC1, @USUARIO = @USR, @CLIENTE = @CLI
    ,@TEXTO = N'El nivel esta justo en la marca inferior. No relleno, aviso.'
    ,@DICTADO_UUID      = '9E1C3A70-0002-4A00-9000-000000000001'
...
/* -- 7. TAR-002 exige foto: no se cierra sin ella -- */
...
DECLARE @OC3 INT, @TEJ3 INT
DECLARE @U3 UNIQUEIDENTIFIER = '9E1C3A70-0003-4A00-9000-000000000001'
DECLARE @UF UNIQUEIDENTIFIER = '9E1C3A70-0004-4A00-9000-000000000001'

SELECT @OC3 = toc.[toc_id] FROM [dbo].[Tarea_Ocurrencia] toc
  JOIN [dbo].[Tarea] t ON t.[tar_id] = toc.[toc_tarea] WHERE t.[tar_codigo] = N'TAR-002'

PRINT ''
PRINT '== EMPEZAR TAR-002 (pide foto) =='
EXEC [dbo].[API_UPS_TAREA_EJECUCION]
     @UUID = @U3, @USUARIO = @USR, @CLIENTE = @CLI, @OCURRENCIA = @OC3

SELECT @TEJ3 = [tej_id] FROM [dbo].[Tarea_Ejecucion] WHERE [tej_uuid] = @U3

PRINT '== CERRAR SIN FOTO (debe fallar) =='
BEGIN TRY
    EXEC [dbo].[API_UPS_TAREA_EJECUCION]
         @UUID = @U3, @USUARIO = @USR, @CLIENTE = @CLI
        ,@OCURRENCIA = @OC3, @FINALIZAR = 1, @CONFORME = 1
    PRINT '  MAL: cerro sin foto'
END TRY
BEGIN CATCH
    PRINT '  rechazado: ' + ERROR_MESSAGE()
END CATCH

PRINT '== SUBIR LA FOTO, Y REENVIARLA =='
EXEC [dbo].[API_INS_EVIDENCIA]
     @UUID = @UF, @USUARIO = @USR, @CLIENTE = @CLI
    ,@DESTINO = N'TAREA', @DESTINO_ID = @TEJ3
    ,@NOMBRE_ORIGINAL = N'filtro.jpg', @NOMBRE_ALMACENADO = N'9e1c3a70-0004.jpg'
    ,@RUTA = N'sigma/tareas/9e1c3a70-0004.jpg', @BYTE = 482113
    ,@ANCHO = 1440, @ALTO = 1080, @DISPOSITIVO = N'Moto G84 - demo'
...
PRINT '== CERRAR CON FOTO =='
EXEC [dbo].[API_UPS_TAREA_EJECUCION]
     @UUID = @U3, @USUARIO = @USR, @CLIENTE = @CLI
    ,@OCURRENCIA = @OC3, @FINALIZAR = 1, @CONFORME = 1
    ,@RESULTADO = N'Filtro soplado, sin saturacion.'


/* -- 8. Como quedo todo -- */
PRINT ''
PRINT '== RESULTADO =='
SELECT t.[tar_codigo], t.[tar_requiere_evidencia] AS [PIDE_FOTO]
      ,toe.[toe_nombre] AS [ESTADO]
      ,tej.[tej_conforme], tej.[tej_duracion_minuto], tej.[tej_resultado]
      ,(SELECT COUNT(*) FROM [dbo].[Archivo_Vinculo] av
         WHERE av.[avi_tarea_ejecucion] = tej.[tej_id]) AS [FOTOS]
  FROM [dbo].[Tarea_Ocurrencia]        toc
...
EXEC [dbo].[API_SEL_TAREA_COMENTARIO] @USUARIO = @USR, @CLIENTE = @CLI, @ID = @OC1

PRINT ''
PRINT '== DICTADOS REGISTRADOS =='
SELECT [dvo_id], [dvo_texto], [dvo_confianza], [dvo_duracion_segundo]
      ,[dvo_intentos], [dvo_proceso_estado], [dvo_archivo]
  FROM [dbo].[Dictado_Voz]
GO
