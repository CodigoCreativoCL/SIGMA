...
-- DESCRIPTION:     LOS VALORES DE CATALOGO BAJAN CON LA SABANA.
-- =============================================
-- El bloque 3 bajaba las CABECERAS de los catalogos y ningun valor, y
-- `valoresDe()` no tenia respaldo en disco. Resultado: sin senal, toda hoja con
-- chips de catalogo se quedaba sin opciones. El **selector de estado del activo
-- (HU-038)** y la **severidad de la bitacora** son las dos que bloquean: no se
-- puede elegir de una lista vacia, y las dos capturas se hacen justo donde no
-- hay senal.
--
-- Es el mismo problema que los motivos de cierre tenian y que resolvio el 190;
-- este lo resuelve para TODOS los catalogos de una vez.
--
-- POR QUE UN SEGUNDO RESULTADO Y NO UN BLOQUE 10
--   Las cabeceras y sus valores son el mismo dato y se piden juntos. El patron
--   ya existe: el bloque 6 trae repuestos, bodegas, ubicaciones y tipos.
--
-- CUIDADO AL LEER ESTO DESDE LA APP
--   El bloque 3 pasa de UN resultado a DOS, asi que la sincronizacion deja de
--   guardarlo como `CATALOGOS` y pasa a `CATALOGOS_0` y `CATALOGOS_1`. La
--   constante `CacheDatos.catalogos` se actualiza en el mismo commit: si no,
--   seguiria leyendo una entidad que ya nadie escribe y devolveria vacio SIN
--   FALLAR, que es el error mas caro de encontrar.
--
-- El resto del SP se conserva EXACTO: este script se genero parcheando la
-- definicion viva, no reescribiendola a mano.
-- =============================================
SET NOCOUNT ON
GO
...
BEGIN
    ...
    -- ahi puede no haber señal. Pidiendolos por red, sin señal la hoja no
