...
-- DESCRIPTION:     LOS MOTIVOS DE CIERRE DE OT BAJAN CON LA SABANA (HU-120).
-- =============================================
-- El cierre de una OT quedo encolandose bien sin señal, pero la hoja pedia los
-- motivos POR RED: sin conexion no aparecia ningun chip y no se podia cerrar.
-- El encolado offline no sirve de nada si no se puede llegar a el.
--
-- Es el mismo problema que los tipos y estados de permiso ya tenian resuelto
-- bajando en el bloque 8. Este script agrega el bloque 9 con la misma forma:
-- una linea en el manifiesto y un SELECT por @TIPO.
--
-- Se agrega un bloque nuevo en vez de colgarlo de CATALOGOS porque aquel lee
-- el sistema generico Catalogo/Catalogo_Valor y estos motivos tienen tabla
-- propia. El bloque queda abierto para los demas catalogos de OT.
--
