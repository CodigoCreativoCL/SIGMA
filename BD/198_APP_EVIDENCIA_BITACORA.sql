...
-- DESCRIPTION:     LA BITACORA ACEPTA EVIDENCIA, COMO LA TAREA Y LA OT.
-- =============================================
-- LA COLUMNA YA ESTABA
--
--   Archivo_Vinculo es polimorfica a proposito y tiene avi_bitacora desde el
--   principio; su propio comentario la nombra entre las cosas a las que se le
--   puede colgar un archivo. Lo que faltaba era la rama de @DESTINO en los dos
--   SP: el de alta la rechazaba con «ese destino de evidencia no existe» y el
--   de consulta devolvia vacio.
--
--   Asi que una anotacion de bitacora no podia llevar una foto, y es justo
--   donde mas hace falta: se escribe delante de la fuga, no despues.
--
-- LOS DOS SP, NO UNO
--
--   Agregar solo el alta habria dejado archivos guardados que nadie puede
--   listar: colgados de una entrada, invisibles en su ficha. Peor que no
--   poder subirlos.
--
-- SE APLICA CON -f 65001
--
--   Los dos SP llevan acentos en sus comentarios y en sus mensajes. Sin eso
--   sqlcmd los lee como ANSI y entran roto sin que nada avise.
-- =============================================
SET NOCOUNT ON
GO
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @DES NOT IN (N'TAREA', N'ORDEN', N'PASO', N'RESPUESTA',
                        N'FALLA', N'HALLAZGO', N'ACTIVO', N'BITACORA')
        BEGIN
...
-- ---------------------------------------------------------------------------
-- Verificacion: la rama existe en los dos
-- ---------------------------------------------------------------------------
SELECT CASE WHEN OBJECT_DEFINITION(OBJECT_ID('API_INS_EVIDENCIA')) LIKE '%avi_bitacora%'
             AND OBJECT_DEFINITION(OBJECT_ID('API_SEL_EVIDENCIA')) LIKE '%avi_bitacora%'
            THEN 'BITACORA aceptada en el alta y en la consulta'
            ELSE 'FALTA en alguno de los dos' END AS RESULTADO
GO
