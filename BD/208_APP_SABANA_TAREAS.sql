...
-- DESCRIPTION:     LAS TAREAS BAJAN CON LA SABANA (HU-103, HU-104).
-- =============================================
-- Bryan: «necesito que cuando no tenga conexion tambien pueda responder mi
-- tarea asignada».
--
-- Responder ya se encolaba; lo que faltaba era poder LLEGAR a la pantalla. La
-- bandeja de tareas se pedia por red en cada apertura, asi que sin senal salia
-- vacia: el tecnico caminaba hasta el equipo sin saber que le tocaba.
--
-- Es el mismo agujero que tuvieron los motivos de cierre (190) y los valores de
-- catalogo (192): la escritura offline no sirve si la lectura que la precede
-- no lo es.
--
-- El bloque DELEGA en API_SEL_TAREA en vez de repetir su SELECT: ese SP ya
-- resuelve la foto del activo, el favorito, la linea y el filtro de plantas.
-- Dos copias serian dos verdades.
-- =============================================
SET NOCOUNT ON
GO
...
--
--   La app tiene que poder trabajar SIN SE�AL. Para eso baja una vez todo lo
