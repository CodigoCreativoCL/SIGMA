...
AS
BEGIN
    IF [dbo].[FNC_USUARIO_TIENE_PERMISO](@USUARIO, @CLIENTE, NULL, N'CERRAR OT') = 1
        RETURN 1
    RETURN 0
END
GO
...
IF NOT EXISTS (SELECT 1 FROM [dbo].[Permiso] WHERE [prm_codigo] = 'CERRAR OT')
    INSERT INTO [dbo].[Permiso] ([prm_codigo], [prm_nombre], [prm_modulo], [prm_permiso_ambito], [prm_descripcion], [prm_usuario_creacion])
    VALUES ('CERRAR OT', 'Cerrar órdenes de trabajo', 'ORDEN TRABAJO', 1,
            'Planificador, supervisor de mantenimiento y jefe de mantenimiento. El técnico finaliza, pero no cierra.', 1)
GO
IF NOT EXISTS (SELECT 1 FROM [dbo].[Permiso] WHERE [prm_codigo] = 'ADJUNTAR OT EXTERNA')
    INSERT INTO [dbo].[Permiso] ([prm_codigo], [prm_nombre], [prm_modulo], [prm_permiso_ambito], [prm_descripcion], [prm_usuario_creacion])
    VALUES ('ADJUNTAR OT EXTERNA', 'Adjuntar la OT del proveedor externo', 'ORDEN TRABAJO', 1,
            'El jefe de mantenimiento adjunta el informe que la empresa externa envía por correo.', 1)
GO
IF NOT EXISTS (SELECT 1 FROM [dbo].[Permiso] WHERE [prm_codigo] = 'GESTIONAR STOCK')
    INSERT INTO [dbo].[Permiso] ([prm_codigo], [prm_nombre], [prm_modulo], [prm_permiso_ambito], [prm_descripcion], [prm_usuario_creacion])
    VALUES ('GESTIONAR STOCK', 'Definir mínimo y máximo de stock', 'INVENTARIO', 1,
            'Del bodeguero. Define niveles de reposición; no compra ni autoriza compras.', 1)
GO
IF NOT EXISTS (SELECT 1 FROM [dbo].[Permiso] WHERE [prm_codigo] = 'AGREGAR COMPANERO ACTIVIDAD')
    INSERT INTO [dbo].[Permiso] ([prm_codigo], [prm_nombre], [prm_modulo], [prm_permiso_ambito], [prm_descripcion], [prm_usuario_creacion])
    VALUES ('AGREGAR COMPANERO ACTIVIDAD', 'Sumar técnicos a una actividad tomada', 'ORDEN TRABAJO', 2,
            'Quien tomó la actividad abierta puede registrar con quiénes la realizó.', 1)
GO
