/* ============================================================================
   SIGMA — Bloque 239
   EL FILTRO POR TIPO DE ACTIVO INCLUYE LOS SUBTIPOS               HU-030 #1
   ----------------------------------------------------------------------------

   El criterio dice: "cuando creo el tipo Blower dentro de Equipo rotatorio,
   al filtrar por Equipo rotatorio se incluyen los activos de tipo Blower".
   SEL_ACTIVO comparaba act_activo_tipo = @ACTIVO_TIPO, asi que el padre no
   traia nada. Se arma la lista de tipo + descendientes con un CTE recursivo
   antes del WHERE dinamico, igual que ya se hace con las areas (@AREAS).

   Detectado el 17-09-2026 al preparar la evidencia del Sprint 2. Es el SP
   completo con el bloque nuevo, tomado de la definicion vigente.
   ============================================================================ */
USE [db_acd593_sigma]
GO
SET ANSI_NULLS ON
...
--TIPO + SUBTIPOS (HU-030 #1: filtrar por el tipo padre incluye a los hijos)
DECLARE @TIPOS VARCHAR(MAX) = NULL
IF (@ACTIVO_TIPO IS NOT NULL)
BEGIN
    DECLARE @tip TABLE (id INT);
    ;WITH tip AS (
        SELECT ati_id FROM [dbo].[Activo_Tipo] WHERE ati_id = @ACTIVO_TIPO
        UNION ALL
        SELECT h.ati_id FROM [dbo].[Activo_Tipo] h
        INNER JOIN tip ON h.ati_activo_tipo_padre = tip.ati_id
    )
    INSERT INTO @tip (id) SELECT ati_id FROM tip;

    SELECT @TIPOS = STUFF((SELECT ',' + CAST(id AS VARCHAR(10)) FROM @tip FOR XML PATH('')), 1, 1, '');
END
...
BEGIN
    ...
    IF (@TIPOS IS NOT NULL) BEGIN
        SET @WHERE = @WHERE + ' AND act.act_activo_tipo IN (' + @TIPOS + ') '
    END
...
PRINT '--- SEL_ACTIVO filtra por tipo y subtipos (bloque 239).'
GO
