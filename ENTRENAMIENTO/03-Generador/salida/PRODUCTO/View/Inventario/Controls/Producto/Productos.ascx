<%--
    USERCONTROL DE LISTADO (GRID) - Productos.ascx
...
--%>
<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Productos.ascx.cs" Inherits="View_Inventario_Controls_Producto_Productos" %>
<%@ Register Src="~/View/Comun/Controls/FiltroAvanzado.ascx" TagPrefix="wuc" TagName="Filtro" %>

<script type="text/javascript">

    // Abre el formulario de Producto. 'query' llega ya cifrado desde el code-behind.
    function abrirProducto(query) {
        window.location = ('<%=ResolveUrl(URLNuevoProducto) %>?query=' + query);
    }
...
<asp:UpdatePanel runat="server" ID="udPanel" UpdateMode="Conditional">
    <ContentTemplate>
        ...
        <rad:RadGrid2 ID="Grid" runat="server"
            OnItemDataBound="rgrProductos_ItemDataBound">
