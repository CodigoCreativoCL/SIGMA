<%--
    USERCONTROL DE LISTADO (GRID) - Usuarios.ascx

    PATRON (ver 00-Patrones-Originales/PATRON_MVC.md seccion 4 y PATRON_CONTROLES.md seccion 1):
      - El listado NUNCA vive en el .aspx: vive en un UserControl reutilizable.
      - El grid va SIEMPRE dentro de un asp:UpdatePanel UpdateMode="Conditional"
        para poder refrescar con udPanel.Update() sin recargar toda la pagina.
      - Los botones de accion (Nuevo / Deshabilitar) van en el CommandItemTemplate.
      - La navegacion al formulario se hace por JavaScript con un querystring CIFRADO.
--%>
<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Usuarios.ascx.cs" Inherits="View_Seguridad_Controls_Usuario_Usuarios" %>
<%@ Register Src="~/View/Comun/Controls/FiltroAvanzado.ascx" TagPrefix="wuc" TagName="Filtro" %>

<script type="text/javascript">

    // Abre el formulario de Usuario. 'query' llega ya cifrado desde el code-behind.
    // URLNuevoUsuario es una propiedad publica del UserControl que setea la pagina padre.
    function abrirUsuario(query) {
        window.location = ('<%=ResolveUrl(URLNuevoUsuario) %>?query=' + query);
    }

    // Refresca el grid via AJAX sin recargar la pagina.
    // Lo habilita Tools.tools.RegisterPostBackScript(Grid) en el code-behind.
    function refresh() {
        __doPostBack("<%=Grid.ClientID %>", '');
    }

</script>

<%-- Barra de filtros: control comun del proyecto, no se reescribe por pantalla --%>
<wuc:Filtro ID="wucFiltro" runat="server" />

<asp:UpdatePanel runat="server" ID="udPanel" UpdateMode="Conditional">
    <ContentTemplate>

        <rad:RadGrid2 ID="Grid" runat="server"
            OnItemDataBound="rgrUsuarios_ItemDataBound">

            <%-- DataKeyNames: columnas que luego se leen con GetDataKeyValue(...) --%>
            <MasterTableView CommandItemDisplay="Top" DataKeyNames="usu_id">

                <CommandItemTemplate>
                    <div style="margin-bottom: 5px;">

                        <%-- Solo navega por JS: no necesita OnClick de servidor --%>
                        <asp:LinkButton ID="lnkNuevo" runat="server" Text="Nuevo"
                            CssClass="icono_guardar"
                            OnClientClick="abrirUsuario(0)" />

                        <%-- ConfirSweetAlert devuelve false si el usuario cancela:
                             el "return" evita el postback y por lo tanto el OnClick. --%>
                        <asp:LinkButton ID="lnkDeshabilitar" runat="server" Text="Deshabilitar"
                            CssClass="icono_eliminar"
                            OnClick="lnkDeshabilitar_Click"
                            OnClientClick="return ConfirSweetAlert(this, '', 'Esta seguro que desea deshabilitar los usuarios seleccionados?');" />

                    </div>
                </CommandItemTemplate>

            </MasterTableView>

        </rad:RadGrid2>

    </ContentTemplate>
</asp:UpdatePanel>
